...
def build_payload(msg, ctx):
    ...
    if msg_type == 'text':
        ...
        if not media_id:
            return None, "无法获取图片"
        if is_telegram:
            from telegram_bot import download_telegram_media
            data, content_type = download_telegram_media(media_id)
        else:
        ...
        recognized_text = ""
        if is_telegram:
            # Telegram 语音是 OGG/Opus 格式，尝试 ASR（可能需要转码）
            recognized_text = recognize_voice(data, voice_format="ogg") or ""
        else:
